...
export async function PATCH(
  ...
  try {
    ...
    const productSnapshot = await prismadb.product.findUnique({
      where: {
        id: params.productId
      },
      include: {
    ...
    if (productSnapshot?.images) {
      const imageUrls = images.map((image: { url: string }) => image.url);
      const imagesToDelete = productSnapshot.images.filter(
        (snapshotImage) => !imageUrls.includes(snapshotImage.url)
      );

      imagesToDelete.forEach((image) => {
        deleteCloudImage(image.url);
      });
    }
...
export async function DELETE(
  ...
  try {
    ...
    const deleteResp = await prismadb.product.deleteMany({
      where: { id: params.productId }
    });
